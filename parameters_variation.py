...
def get_loaders(scenario):
    # Cifar10 dataset
    if scenario['dataset'] == 'cifar10':
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=scenario["batch_size"], shuffle=True, num_workers=2)
    testloader = torch.utils.data.DataLoader(
        testset, batch_size=scenario["batch_size"], shuffle=False, num_workers=2)
...
def train_epoch(model, data_loader, criterion, optimizer, pert_argmax):
    model.train()
    running_loss = 0.0
    ...
    for data in data_loader:
        ...
        # If using argmax, we need to convert single index
        # into one-hot of length 10
...
def test(model, data_loader, save_file):
    model.eval()
    ...
    print('Total accuracy: %2d %%' % (100 * total_correct / total))
    scores = [100*class_correct[i]/class_total[i] for i in range(10)] + [acc]
    with open(save_file + 'scores.pickle', 'wb') as f:
        pickle.dump(scores, f)


def run_scenario(scenario):
    save_file = f'./results/{scenario["dataset"]}/{scenario["noise"]}_{scenario["sigma"]}/mode{scenario["mode"]}/{scenario["epochs"]}_{scenario["batch_size"]}/'
    os.makedirs(save_file, exist_ok=True)

    BATCH_SIZE = scenario['batch_size']
    EPOCHS = scenario['epochs']

    trainloader, testloader = get_loaders(scenario)
    pert_argmax = perturbations.perturbed(
        argmax, num_samples=100, sigma=scenario['sigma'],
        noise=scenario['noise'], batched=True, device=device
    )
    mode_choices = [0, 1, 2]
    parser = argparse.ArgumentParser(description='Perturbations Demo.')
    parser.add_argument(
        "--mode", dest='mode', choices=mode_choices, default=scenario['mode'], type=int,
    ...
    for epoch in range(EPOCHS):
        loss = train_epoch(model, trainloader, criterion, optimizer, pert_argmax)
        print('Epoch {:>3}, Loss: {:.4f}'.format(epoch+1, loss))
        loss_history.append(loss)
    with open(save_file + 'loss.pickle', 'wb') as f:
        pickle.dump(loss_history, f)
    # Test
    test(model, testloader, save_file)

def run_list_scenarios(scenarios_list):
    for dataset in scenarios_list['dataset']:
        for mode in scenarios_list['mode']:
            for noise in scenarios_list['noise']:
                for sigma in scenarios_list['sigma']:
                    for epochs in scenarios_list['epochs']:
                        for batch_size in scenarios_list['batch_size']:
                            scenario = {
                                'dataset': dataset,
                                'mode': mode,
                                'noise': noise,
...
if __name__ == "__main__":
    ...
    #     'batch_size': [32],
    # }
    # run_list_scenarios(scenarios_list)

    scenarios_list = {
        'dataset': ['cifar10', 'cinic10'],  # can be 'cifar10' or 'cinic10
        'mode': [1, 2],
        'noise': ['normal', 'gumbel'],  # can be 'gumbel' or 'normal', used in mode 1 and 2
        'sigma': [0.001, 0.01, 10.0, 100.0],  # used in mode 1 and 2
        'epochs': [50],
        'batch_size': [32],
    }
    run_list_scenarios(scenarios_list)
